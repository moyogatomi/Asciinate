...
def generator_wave():
    ...
    for z in range(200):
        grayscale_frame = np.uint8(
            255
            * np.clip(
                (
                    np.sin(np.sin(z / 50) * x + z / 50) # some sine function
                    + np.sin(np.sin(z / 50) * y + z / 50)
                ),
                0,
                255,
            )
...
tube = Engine(media=media_generator, core=core, show_fps=True)
